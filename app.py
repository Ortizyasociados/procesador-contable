...
if uploaded_files:
    ...
    uploaded = {"uploaded_temp.zip": None}
                ...
                if file.endswith(".xml"):
                    try:
                        ...
                        if invoice_tree is not None:
                            ...
                            if not razon_social_proveedor: razon_social_proveedor = invoice_tree.findtext('.//cac:AccountingSupplierParty//cac:PartyLegalEntity/cbc:RegistrationName', namespaces=ns)
    ...
    total_row = pd.DataFrame([{
        'No_Item': None, 'ID_Factura': 'TOTAL', 'Fecha_Emision': None, 'Fecha_Vencimiento': None, 'Tipo_Pago': None,
        'NIT_Proveedor': None, 'Razon_Social_Proveedor': None, 'Direccion_Proveedor': None, 'Telefono_Proveedor': None,
        'Correo_Proveedor': None, 'Ciudad_Proveedor': None, 'NIT_Adquirente': None, 'Razon_Social_Adquirente': None,
        'Moneda': None, 'Total_Base_Impuestos': df['Total_Base_Impuestos'].sum(), 'Base_Exenta': df['Base_Exenta'].sum(),
        'Total_Impuestos': df['Total_Impuestos'].sum(), 'Otros_Impuestos': df['Otros_Impuestos'].sum(), 'Total_Factura': df['Total_Factura'].sum()
    }])
    ...
    # Base de datos de terceros (Tu lógica intacta)
    sup = df[['NIT_Proveedor', 'Razon_Social_Proveedor', 'Direccion_Proveedor', 'Telefono_Proveedor', 'Correo_Proveedor', 'Ciudad_Proveedor']].copy()
    sup = sup.drop_duplicates().rename(columns={'NIT_Proveedor': 'NIT_Tercero', 'Razon_Social_Proveedor': 'Razon_Social_Tercero', 'Direccion_Proveedor': 'Direccion_Tercero', 'Telefono_Proveedor': 'Telefono_Tercero', 'Correo_Proveedor': 'Correo_Tercero', 'Ciudad_Proveedor': 'Ciudad_Tercero'})
    sup['Tipo_Tercero'] = 'Proveedor'
    acu = df[['NIT_Adquirente', 'Razon_Social_Adquirente']].copy()
    acu = acu.drop_duplicates().rename(columns={'NIT_Adquirente': 'NIT_Tercero', 'Razon_Social_Adquirente': 'Razon_Social_Tercero'})
    ...
    output_third = "Base_Datos_Terceros.xlsx"
    third_parties_df.to_excel(output_third, index=False)
    with open(output_third, "rb") as f:
        st.download_button("Descargar Base de Datos Terceros", f, file_name=output_third)
